...
def city_code_spider():
    ...
    for i in range(4, len(city_list[0])):
        ...
        province_name = \
        jsonpath.jsonpath(city_list, '$.*' + str([i]) + '.name')[0]
        # 表示所有的城市
        # for j in range(len(province_list[0]['subLevelModelList'])):
        # 表示所有的省会城市
        for j in [0]:
            city_dic = {}
            city_name = jsonpath.jsonpath(city_list, '$.*' + str(
                [i]) + '.subLevelModelList' + str([j]) + '.name')[0]
            city_code = jsonpath.jsonpath(city_list, '$.*' + str(
                [i]) + '.subLevelModelList' + str([j]) + '.code')[0]
    ...
    if not get_end_url_exit:
        while not city_code_queue.empty():
            ...
            for page in range(1, 11):
                ...
                try:
                    r = requests.get(url, headers=get_end_url_headers,
                ...
                try:
                    ...
                    # 获取Jobid
                    jobids = result.xpath(
                        '//li//h3[@class="name"]/a/@data-jobid')
                    # print(jobids)
                    # 获取末尾url
                    end_urls = result.xpath(
                        '//li//div[@class="info-primary"]//h3/a/@href')
                    count = 0
                    for end_url in end_urls:
                        job_id = jobids[count]
                        count += 1
                        end_url_dic = {}
                        end_url_dic['province_name'] = city_code_list[
                            'province_name']
...
def get_job_detail(end_url_queue, headers, proxies):
    ...
    while not end_url_queue.empty():
        ...
        try:
            r = requests.get(url=detail_url, headers=get_job_detail_headers,
        ...
        # 获取发布时间
        try:
            job_times = result.xpath('//div[@class="job-author"]/span/text()')[
                0]
        ...
        # 工作地点
        try:
            job_address = result.xpath('//div[@class="info-primary"]/p/text()')[
                0]
        except:
            job_address = ''
        # 工作经验
        try:
            job_experience = \
            result.xpath('//div[@class="info-primary"]/p/text()')[1]
        except:
            job_experience = ''
        # 工作学历
        try:
            job_education = \
            result.xpath('//div[@class="info-primary"]/p/text()')[2]
        except:
            job_education = ''
        # 工作描述
        try:
            job_descriptions = result.xpath(
        ...
        # 团队介绍
        try:
            team_introduces = result.xpath(
                '//div[@class="detail-content"]/div[@class="job-sec"]/div[@class="job-tags"]/span/text()')
        ...
        # 公司介绍
        try:
            company_introduce = result.xpath(
                '//div[@class="detail-content"]/div[@class="job-sec company-info"]/div[@class="text"]')[
                0].text.strip()
        except:
            company_introduce = ''

        params = [job_id, job_name, job_time, job_salary, job_address,
                  job_experience, job_education, job_description]
        try:
            conn = pymysql.connect(host='localhost', port=3306,
                                   db='Boss_spider', user='root',
                                   passwd='591777', charset='utf8')
            cs1 = conn.cursor()
            count = cs1.execute(
                "insert into job_detail(job_id,job_name,job_time,job_salary,job_address,job_experience,job_education,job_description) values(%s,%s,%s,%s,%s,%s,%s,%s)",
                params)
            print(count)
            conn.commit()
            cs1.close()
...
if __name__ == "__main__":
    ...
    # #f = open('job_list.txt','a')
    for i in range(2):
        p1.apply_async(get_end_url,
                       (city_code_queue, end_url_queue, headers, proxies))
        print(1)
