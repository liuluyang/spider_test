...
MYSQL = {'host':'', 'port':3306, 'db':'', 'user':'',
         'passwd':'', 'use_unicode':True}


class ConnectDB(object):
    def __init__(self):
        self.connect = pymysql.connect(**MYSQL, cursorclass=pymysql.cursors.DictCursor)
    ...
    def weibo_info_get(self, group_id=1):
        sql = 'select *from uce_spider_guanzhu where group_id={} order by published_at desc'.format(group_id)
        self.cursor.execute(sql)
